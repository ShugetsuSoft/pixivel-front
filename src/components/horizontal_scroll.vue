<template>
  <div class="horizontal-scroll" @wheel="wheel" ref="scroll" @scroll="onscroll">
    <RouterA
      :to="{ name: 'Detail', params: { id: illust.id } }"
      class="scroll-item"
      v-for="illust in illustsFiltered"
      :key="illust.id"
...
export default {
  ...
  methods: {
    scrollTo(pos) {
      ...
      if (this.scrollPos > this.$refs.scroll.scrollWidth)
        this.scrollPos = this.$refs.scroll.scrollWidth;
      this.$refs.scroll.scrollTo({
        left: this.scrollPos,
      });
      if (
        this.$refs.loadSpan[0].offsetLeft - this.$refs.scroll.scrollLeft <=
    ...
    wheel(event) {
      ...
      if (this.scrollPos > this.$refs.scroll.scrollWidth)
        this.scrollPos = this.$refs.scroll.scrollWidth;
      if (
        (this.scrollPos >= 0 && event.deltaY > 0) ||
        (this.scrollPos <= this.$refs.scroll.scrollWidth && event.deltaY < 0)
      ) {
