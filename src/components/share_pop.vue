<template>
  <div class="box has-text-centered">
    <b-field label="分享链接">
      ...
      <p class="control">
        <b-button class="button is-info" id="copy" data-clipboard-target="#copyInput">复制</b-button>
      </p>
    </b-field>
    <label class="label">直接发送</label>
    <div class="social-container">
      <a class="social-icon-box" v-for="(s, i) in social" :key="i" :style="{'background-color': s.color}" v-html="s.logo" :href="s.url" target="_blank"></a>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.clipboard.on("error", () => {
      this.$buefy.snackbar.open({
        message: '好像，浏览器都不支持复制？不过没关系，可以全选框里面的链接然后复制哦',
        type: 'is-error',
        position: 'is-top',
      })
    })
