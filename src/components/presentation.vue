	...
	<div ref="presentation">
        ...
        <div class="loading-overlay" :class="{'is-active': loading}">
          ...
          <div class="loading-icon"></div>
				...
				<img :src="CurrentImgUrl" :alt="id + '-' + pageCount" :class="{'onepx-blur': loading}"/>
			</div>
		</div>
      ...
      loadUgoira() {
        ...
        this.axios.get(url, {
          ...
          file.loadAsync(response.data).then(() => {
            Promise.all(this.ugoiraFrames.map((item) => {
              return new Promise((resolve, reject) => {
                file.file(item["file"]).async("blob").then((data) => {
                  this.ugoiraImages[item["file"]] = URL.createObjectURL(data)
                  resolve()
                }).catch(error => {
                  reject(error)
                })
